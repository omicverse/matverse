...
BLOCKED: Dict[str, str] = {
    ...
    "analysis.defects.corrections.kumagai":
        "the blocker is data, and stating that first matters because the "
        "install situation is not stable enough to record. This entry once "
        "said pip could not place pydefect because scikit-image had no wheel "
        "for glibc 2.17; scikit-image later arrived as somebody else's "
        "dependency, and the obstacle moved to a pyarrow version reached "
        "through vise and mp-api - which is itself present on some of the "
        "interpreters here and absent on others. conda-forge installs "
        "pydefect cleanly on any of them, which was checked rather than "
        "assumed, and with pydefect present get_efnv_correction imports and "
        "runs. So the install is a nuisance that varies, and what actually "
        "stops this is the data: it maps each defect site onto "
